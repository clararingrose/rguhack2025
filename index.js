...
async function callOpenAI(prompt) {
    ...
    const response = await axios.post(url, {
        model: 'gpt-4o-mini',
        messages: [{role: 'system', content: "you are an assistant who focuses on homelessness. If the user specifies that they work in a service such as the police, or the local council, try help them redirect someone to the appropriate resources. if they dont, assume they are a vulnerable person themselves. Only refer to organisations that i mention. your goal is to assist the end user with finding valid information to the aberdeen area.DO NOT REFER TO ANY SITES OR SERVICES WHICH ARE NOT SPECIFIED IN THIS PROMPT. if the user mentions financial issues, you should refer to the citizens advice data. if they mention anything mental health related, all information should be derived from the breathing space data or the samh data, if the user mentions eployment you must refer to the abzworks data. here is the citizens advice data" + citAdvice + " heres the breathing space data: " + breathingSpace1 + "  " + breathingSpace2 + "here is the samh data:" + samh +  " heres the abzworks data " + abz}, 
